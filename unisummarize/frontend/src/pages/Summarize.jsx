  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (inputType === 'text' || inputType === 'url') {
        ...
        response = await fetch(`${BACKEND_URL}/api/summarize/file`, {
          method: 'POST',
          body: formData,
        });
      } else {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
          ...
          <div className="animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
            <div className="sticky top-24">
              ...
              <div className="max-h-[600px] overflow-y-auto bg-gray-100 rounded-3xl p-8 shadow-lg border border-gray-300">
                {isLoading ? (
                  <div className="flex flex-col items-center justify-center h-full text-center p-6">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-4 border-gray-700 mb-5"></div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Generating summary...</h3>
                  </div>
                ) : result ? (
                  <div className="space-y-6">
                    <button
                      onClick={() => setShowRaw(!showRaw)}
                      className="mb-6 text-gray-700 hover:underline"
                    >
                      {showRaw ? 'Hide Raw Summary HTML' : 'Show Raw Summary HTML'}
                    </button>
                    {showRaw && (
                      <pre className="whitespace-pre-wrap max-h-64 overflow-y-auto bg-white p-6 rounded-2xl text-sm mb-6 border border-gray-300 shadow-sm">
                        {result}
                      </pre>
                    )}
                    <div className="bg-white rounded-2xl p-8 shadow-md prose max-w-none text-gray-900 leading-relaxed">
                      <div dangerouslySetInnerHTML={{ __html: result }} />
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full text-center p-6">
                    <div className="w-24 h-24 bg-white rounded-2xl flex items-center justify-center mb-8 shadow-md border border-gray-300">
                      <FileText className="h-12 w-12 text-gray-400" />
                    </div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Ready to Summarize</h3>
                    <p className="text-gray-800">Enter your content and click "Generate Summary" to get started.</p>
                  </div>
                )}
              </div>
            </div>
